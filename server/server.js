...
/* =========================================================================
   CATÁLOGO — mora em lib/catalogo.js porque o cron também precisa dele
   (nome e foto do produto nos e-mails de carrinho esquecido e de campanha).
   Os nomes seguem valendo exatamente como antes no resto deste arquivo.
========================================================================= */
const {
  CAIXA_PADRAO, buildPackage,
  PRODUCTS, BUILTIN_CATEGORIES, PRODUCT_CATEGORIES, PRODUCT_BADGES,
  CUSTOM_PRODUCT_ID_START, CATEGORIAS_APOSENTADAS,
  categoriasEmUso, getAllCategories, isValidCategorySlug,
  getProductOverridesMap, getAllProductIds, photosFromRow, effectiveProduct,
...
function produtosDaCampanha(campanha){
  ...
  return ids.map(id => {
    ...
    return {
      ...
      photoUrl: p.photoUrl,
      // CLIENT_ORIGIN, não SITE_URL: é o endereço que todos os outros links
      // de e-mail usam, e a mesma campanha não pode gerar links diferentes
      // conforme quem dispara o lote (painel ou cron).
...
app.get("/api/interno/tarefas-periodicas", cronLimiter, async (req, res) => {
  ...
  // ⚠️ Comparação em BYTES, não em caracteres: timingSafeEqual exige buffers
  // do mesmo tamanho, e "a".length === "á".length enquanto os bytes diferem —
  // um token com acento estourava RangeError e virava 500 em vez de 401.
  const recebido = Buffer.from(String(req.query.token || ""));
  const esperado = Buffer.from(segredo);
  const bateu = recebido.length === esperado.length
